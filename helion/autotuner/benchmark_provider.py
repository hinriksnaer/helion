...
class BenchmarkProvider(abc.ABC):
    ...
    Lifecycle::
        ...
        try:
            provider.benchmark_function(config, fn)  # benchmark a compiled config
        finally:
...
class LocalBenchmarkProvider(BenchmarkProvider):
    ...
    def __init__(
        ...
        # TODO(hinriksnaer): this likely doesn't belong in the provider, but it's convenient
        # to compute it here since it requires running the kernel's default config. Refactor
        # later so provider is only responsible for "run and time" and not accuracy validation or arg management.
